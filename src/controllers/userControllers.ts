...
export const userMakeOrder = async (req:JwtPayload, res:Response, next:NextFunction) => {
    try {
        ...
        const vendor:any = await VendorInstance.findOne({where: {id: food.vendorId}}) //as unknown as VendorAttributes
        if(!vendor){
            return res.status(400).json({
                status: "error",
                method: req.method,
        ...
        for (const food of food_items) {
            const foodInstance:any = await FoodInstance.findOne({ where: { id: food.id } });
            if (foodInstance) {
